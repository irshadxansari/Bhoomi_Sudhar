...
import { ADMIN_ROUTE, AUTH_ROUTES, DEFAULT_LOGIN_REDIRECT, PUBLIC_ROUTES } from "./routes"
...
export async function middleware(request: NextRequest) {
  ...
  if (isAdminRoute) {
    if (!isLoggedIn || userSession?.role !== "ADMIN") return NextResponse.redirect(new URL("/login", request.url))
  }
